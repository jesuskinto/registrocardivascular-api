...
class MongoLib {
    ...
    getAll(collection, query, options = {}, { resPerPage, page } = {}) {
        return this.connect().then(async db => {
            ...
            if (resPerPage && page) {
                const res = await db.collection(collection).find(filterField, options).skip((resPerPage * page) - resPerPage).limit(resPerPage).toArray();
                const count = await db.collection(collection).countDocuments(filterField);
                return { res, count }
            }
            return db.collection(collection).find(filterField, options).toArray();
        });
    }
